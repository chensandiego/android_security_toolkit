...
def process_apks_in_directory(dir_path, label, processed_apks):
    ...
    apk_files_to_process = [
        ...
        if not f.startswith(".")
        and os.path.join(dir_path, f) not in processed_apks
    ]

    if not apk_files_to_process:
        print(f"All APKs in {dir_path} already processed. Skipping.")
        return []

    print(f"Processing {len(apk_files_to_process)} new APKs in {dir_path}...")

    with concurrent.futures.ProcessPoolExecutor() as executor:
        future_to_apk = {
            executor.submit(analyze_single_apk, apk_path): apk_path
            for apk_path in apk_files_to_process
        }

        for future in tqdm(
            concurrent.futures.as_completed(future_to_apk),
            total=len(apk_files_to_process),
            desc=f"Analyzing {label}",
        ):
...
def main():
    ...
    output_file = os.path.join(
        os.path.dirname(__file__), "known_apks_generated.json"
    )
    all_fingerprints = []
    processed_apks = set()
    ...
    if os.path.exists(output_file):
        with open(output_file, "r") as f:
            try:
                ...
                print(
                    f"Loaded {len(all_fingerprints)} existing fingerprints. {len(processed_apks)} APKs already processed."
                )
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        future_to_label = {
            executor.submit(
                process_apks_in_directory,
                os.path.join(DATASET_BASE_DIR, category_dir),
                label,
                processed_apks,
            ): label
        ...
        for future in concurrent.futures.as_completed(future_to_label):
            ...
            try:
                category_fingerprints = future.result()
                if category_fingerprints:
                    all_fingerprints.extend(category_fingerprints)
                    print(
                        f"Saving {len(all_fingerprints)} total fingerprints to {output_file} after processing '{label}'..."
                    )
                    with open(output_file, "w") as f:
                        json.dump(all_fingerprints, f, indent=4)
            except Exception as e:
                print(f"Error processing category for label {label}: {e}")

    print(f"Final save of {len(all_fingerprints)} fingerprints to {output_file}...")
    with open(output_file, "w") as f:
        json.dump(all_fingerprints, f, indent=4)
